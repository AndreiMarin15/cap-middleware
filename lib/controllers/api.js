...
const api = {
  ...
  insertEncounterData: async (req, res) => {
    ...
    const validApiKey = await validateApiKey(apiKey);
    if (validApiKey !== false) {
      await client().from("encounter").insert(req.body.data);
  ...
  addVitals: async (req, res) => {
    // const data = await transformObject(req.body.data, req.body.table);
    console.log(JSON.stringify(req.body.data) + "hiiiii");
    ...
    if (validApiKey !== false) {
      ...
      res.send(newObject);
      const newIds = [];
      const observations = await client().from("observation").select("*");

      // console.log("query", observations);
      // obs.forEach(async (observation) => {
      ...
      //     })
      //     .select("*");
      //   console.log("id data 0", id.data[0].id);
      // });
      ...
      // Now you can proceed with the update after all insert operations are completed
      ...
      console.log(encounter.data[0].resource);
    } else {
      res.status(401).json({ message: "Unauthorized" });
    }
    // res.json(response);
