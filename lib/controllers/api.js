...
function convertDiagnosis(reqBody) {
  console.log(reqBody);
  const post = JSON.parse(reqBody.post);
  const issueRecord = JSON.parse(reqBody.issueRecord);
  if ((issueRecord?.type ?? "medical_problem") === "medical_problem") {
    return {
      id: "finalDiagnosis",
      code: {
        coding: [
      ...
      subject: {
        type: "Patient",
        reference: reqBody.pid,
      },
      participant: {
        type: "Doctor",
        actor: reqBody.actor,
      },
      valueString: post.form_title,
      resource_type: "Observation",
    };
  } else if (issueRecord.type === "medication") {
    return {
      id: post.form_title,
      form: {
        text: "Film-coated tablet",
      },
      note: post.form_comments,
      status: "Active",
      subject: {
        type: "Patient",
        reference: reqBody.pid,
      ...
      requester: {
        agent: {
          reference: reqBody.actor,
        },
      },
      adverseEvent: {
        adverseReaction: post.form_reaction,
      },
      dispenseRequest: {
        validityPeriod: {
          end: post.form_begin,
        ...
        dispenseInterval: post.form_medication.drug_dosage_instructions || "",
      },
      dosageInstruction: [
        {
          doseAndRate: [
            {
              doseQuantity: {
                doseUnit: post.form_medication.drug_dosage_instructions || "",
              },
            },
          ],
        },
      ...
      medicationCodeableConcept: [
        {
          text: post.form_title,
          coding: [
            {
              system: "http://www.nlm.nih.gov/research/umls/rxnorm",
              display: post.form_title,
            },
          ],
        },
      ],
...
function convertCarePlan(input, reference) {
  const carePlanConversion = {
    health_concern: "dietary management",
    goal: "physical activities",
    instructions: "self monitoring",
  };

  const activities = input.new.care_plan_type
    .map((type, index) => {
      if (!["health_concern", "goal", "instructions"].includes(type)) {
        return null;
      }
...
const api = {
  ...
  addFinalDiagnosis: async (req, res) => {
    const apiKey = req.body.apiKey;
    const validApiKey = await validateApiKey(apiKey);

    if (validApiKey !== false) {
      // console.log(convertDiagnosis(req.body));
      // res.send(convertDiagnosis(req.body));

      const data = convertDiagnosis(req.body);
      ...
      const insert = await client()
        .from("observation")
        .insert({ status: "created", resource: data })
        .select("*");

      const observationId = insert.data[0].id;
      console.log("typeee", req.body.issueRecord.type);
      if (
        (req.body.issueRecord?.type ?? "medical_problem") === "medical_problem"
      ) {
        const update = await client()
          .from("encounter")
          .update({
            resource: {
              ...encounter.data[0].resource,
              contained: [
                ...encounter.data[0].resource.contained,
                observationId,
              ],
            },
          })
        ...
        console.log(update);
        res.send(update);
      } else if (req.body.issueRecord.type === "medication") {
        const insert = await client()
