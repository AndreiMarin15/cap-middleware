...
const getOwnerEmail = async (apiKey) => {
  if (apiKey) {
    const email = await client().from("api_keys").select("*").eq("key", apiKey);

    if (email.error === null && email.data.length > 0) {
      return email.data[0].owner_email;
    }
  }
};
...
const testController = {
  ...
  requestApproval: async (req, res) => {
    if (req.body.api_key) {
      ...
      console.log("id", req.body.patient_id);
      const last_encounter = await endotrackerClient()
        .from("encounter")
        .select("*")
        .eq("resource->subject->>reference", req.body.patient_id)
      ...
      console.log(last_encounter);
      const last_visit =
        last_encounter.data != null && last_encounter.data.length > 0
          ? last_encounter.data[0].ts
          : null;

      console.log("visit", last_visit);
      const contents = [];
      const clinic_visits =
        last_visit != null
          ? await client()
      ...
      console.log("clinic_visits", clinic_visits);
      if (clinic_visits.data != null && clinic_visits.data?.length > 0) {
        console.log("clinic visits not null", clinic_visits.data);
  ...
  updateRequest: async (req, res) => {
    try {
      ...
      if (req.body.status === true) {
        ...
        const last_careplan = await endotrackerClient()
          .from("careplan")
          .select("*")
          .eq("resource->subject->>reference", req.body.patient_id)
          .order("ts", { ascending: false })
        ...
        console.log("last_careplan", last_careplan);
        const last_care =
          last_careplan.data.length > 0 ? last_careplan.data[0].ts : null;

        const care_plans = last_care
          ? await client()
              .from("careplan")
              .select("*")
              .eq("resource->subject->>reference", req.body.patient_id)
        ...
        console.log("care_plans", care_plans);

        if (care_plans.data.length > 0) {
          care_plans.data.map(async (care) => {
            const insertCare = await endotracker.insertInto("careplan", care);
            console.log("care");
            console.log(care);

            console.log("insertCare");
            console.log(insertCare);
          });
        }

        const last_medication = await endotrackerClient()
        ...
        console.log("last_medication", last_medication);
        const last_medic =
          last_medication.data.length > 0 ? last_medication.data[0].ts : null;

        const medications = last_medic
          ? await client()
              .from("medicationrequest")
              .select("*")
              .eq("resource->subject->>reference", req.body.patient_id)
        ...
        if (medications.data.length > 0) {
          medications.data.map(async (medicine) => {
            const insertMedicine = await endotracker.insertInto(
              "medicationrequest",
              medicine,
            );
  ...
  mapForClient: async (req, res) => {
    ...
    if (validateApiKey(api_key)) {
      const data = await db.insertInto("client_mapping", {
        ...
        table_mapping: req.body.table_mapping,
      });
  ...
  getMappingsForAPIKey: async (req, res) => {
    const email = await getOwnerEmail(req.body.api_key);

    const { data, error } = await client()
      .from("client_mapping")
      .select("*")
      .eq("owner_email", email)
      .eq("fhir_table", req.body.table);
